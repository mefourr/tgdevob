...
func Test_voiceDurationValidator_Execute_Grpc_Error(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			voiceDurationValidation := NewVoiceDurationValidator(grpc)

			grpc.On("Send", mock.Anything, float32(tt.args.duration)).
				Return(errors.New("some errors")).
				Once()

			err := voiceDurationValidation.Execute(context.Background(), tt.args.duration)
			assert.NotNil(t, err)
		})
	}
}
...
func Test_voiceDurationValidator_Execute_Duration_Zero(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			voiceDurationValidation := NewVoiceDurationValidator(nil)
			err := voiceDurationValidation.Execute(context.Background(), tt.args.duration)
			assert.NotNil(t, err)
