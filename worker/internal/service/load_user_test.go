...
func Test_service_LoadUser_BaseCase(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			userCacheFinder := mocks.NewMockUserCacheFinder(t)
			userFinder := mocks.NewMockUserFinder(t)
			s := NewLoadUserSvc(
				userCacheFinder,
				userFinder,
			)
			...
			got, err := s.Execute(
				tt.args.ctx,
				domain.Message{UpdateId: 1, Request: nil},
				tt.args.key,
			)
...
func Test_service_LoadUser_UnexpectedError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			userCacheFinder := mocks.NewMockUserCacheFinder(t)
			s := NewLoadUserSvc(userCacheFinder, nil)
